...
export const useFetch = (apiPath, queryTerm="") => {
    ...
    useEffect(() => {
        ...
        async function fetchMovieDetail(){
          const responseData = await fetch(`http://www.omdbapi.com/?apikey=${process.env.REACT_APP_API_KEY}&s=${queryTerm}&type=movie`)
        ...
        async function fetchMovies(){
          try {
            for (let page = 1; page <= pagesToFetch; page++) {
              const response = await fetch(`http://www.omdbapi.com/?apikey=${process.env.REACT_APP_API_KEY}&s=${queryTerm}&type=movie&page=${page}`);
